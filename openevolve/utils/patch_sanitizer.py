...
def extract_raw_patch(text: str) -> str:
    ...
    # Debug: log input text preview
    try:
        input_preview = text[:500].replace('\n', '\\n')
        print(f"[sanitizer] Input text preview: {input_preview}", file=sys.stderr)
    except Exception:
        pass
    ...
    try:
        ...
        print(f"[sanitizer] Final patch size: {len(out)} chars, {len(out.splitlines())} lines", file=sys.stderr)
    except Exception as e:
        print(f"[sanitizer] Preview generation error: {e}", file=sys.stderr)

    # Additional validation - ensure it's actually a diff, not a file path
    if out.strip().startswith('/tmp/') or out.strip().startswith('/workspace/'):
        print(f"[sanitizer] ERROR: Output appears to be a file path, not diff content: {out.strip()[:100]}", file=sys.stderr)
        return ""
    
    # Ensure it contains actual diff markers
    out_lines = out.splitlines()
    has_diff_markers = any(line.startswith(('---', '+++', '@@', '+', '-')) for line in out_lines)
    if not has_diff_markers:
        print(f"[sanitizer] ERROR: Output lacks diff markers. Content: {out.strip()[:200]}", file=sys.stderr)
        return ""
