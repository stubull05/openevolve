...
class LLMConfig(LLMModelConfig):
    ...
    # API configuration
    api_base: str = "http://host.docker.internal:8000/v1"
    ...
    models: List[LLMModelConfig] = field(
        default_factory=lambda: [
            LLMModelConfig(name="qwen2.5-coder:7b", weight=0.8),
            LLMModelConfig(name="qwen2.5-coder:7b", weight=0.2),
        ]
    )
...
class PromptConfig:
    ...
    suggest_simplification_after_chars: Optional[int] = (
        50000  # Suggest simplifying if program exceeds this many characters
    )
...
class EvaluatorConfig:
    ...
    # General settings
    timeout: int = 900  # Maximum evaluation time in seconds
    max_retries: int = 3
