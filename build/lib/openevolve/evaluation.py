...
# ----- NEW: Control whether to persist evolved changes -----
PERSIST_CHANGES = os.getenv("EVAL_PERSIST_CHANGES", "1").lower() in ("1", "true", "yes")
...
def _run_pytests() -> int:
    ...
    if not BACKEND_TEST_DIR.exists():
        ...
        return 0
    proc = _run(["pytest", "-q", str(BACKEND_TEST_DIR)], timeout=PY_TIMEOUT)
...
def evaluate(program_code: str, file_path: str = None, suffix: str = None, **kwargs) -> Dict[str, Any]:
    ...
    We write it into the target repo file, run tests (pytest + JS harness + Playwright).
    
    FIXED: Now only restores original content if the evolved version fails tests
    and PERSIST_CHANGES is True (default). This allows successful evolutions to persist.
    """
    ...
    if existed:
        try:
            ...
            print(f"[eval] WARNING: Could not backup original file {tfile}: {e}", flush=True)
            original_text = None

    # Write the evolved program code
    try:
        tfile.write_text(program_code, encoding="utf-8")
        print(f"[eval] Wrote evolved code to {tfile} ({len(program_code)} chars)", flush=True)
    except Exception as e:
        print(f"[eval] ERROR: Could not write evolved code to {tfile}: {e}", flush=True)
        return {"combined_score": 0.0, "error": f"Could not write file: {e}"}
    ...
    # CRITICAL FIX: Only restore original content based on policy
    should_restore = False
    
    if not PERSIST_CHANGES:
        # Never persist changes - always restore (old behavior)
        should_restore = True
        restore_reason = "PERSIST_CHANGES=0"
    elif not tests_passed:
        # Only restore if tests failed (allow successful evolutions to persist)
        should_restore = True
        restore_reason = f"tests failed (score={combined:.2f})"
    else:
        # Tests passed and PERSIST_CHANGES=1, keep the evolved version
        should_restore = False
        print(f"[eval] SUCCESS: Keeping evolved version (score={combined:.2f})", flush=True)

    if should_restore:
        try:
            if existed and original_text is not None:
                tfile.write_text(original_text, encoding="utf-8")
                print(f"[eval] Restored original content due to: {restore_reason}", flush=True)
            elif not existed:
                ...
                try:
                    ...
                    if tfile.exists():
                        os.remove(str(tfile))
                print(f"[eval] Removed created file due to: {restore_reason}", flush=True)
        except Exception as e:
