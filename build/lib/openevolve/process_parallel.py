...
def _run_iteration_worker(
    ...
    try:
        ...
        if _worker_config.diff_based_evolution:
            ...
            # tolerate a deliberate "SKIP"
            if raw.strip().upper().startswith("SKIP"):
                # whatever your code does to skip this iteration
                raise ValueError("Model skipped")

            # sanitize
            try:
                resp_text = extract_raw_patch(raw)
            except Exception:
                # keep original behavior so you still see the warning in logs
                llm_response = raw
